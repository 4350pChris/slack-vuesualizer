...
<template>
  <div class="flex justify-start gap-4 bg-base-100 text-base-content">
    <UserAvatar :key="user.id" class="h-16 md:h-24 lg:h-44 w-16 md:w-24 lg:w-44 rounded-box"
    ...
    <div class="flex flex-col grow">
      ...
      <div class="grid grid-cols-[1.5rem,minmax(0,max-content)] gap-2">
        <template v-if="user.profile.email">
          ...
          <a class="text-sm border-b-2 border-transparent hover:border-primary transition"
            :href="`mailto:${user.profile.email}`">{{ user.profile.email }}</a>
        </template>
